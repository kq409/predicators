...
def fd_plan_from_sas_file(
    ...
    # Reserve wall-clock budget for SAS translation / setup already charged to
    # ``start_time`` (beginning of ``run_task_plan_once``). Without this, the
        ...
        if search_match:
            ...
            if original_search_expr.endswith(")"):
                ...
                kstar_search_flag = kstar_search_flag.replace(
                    ...
                    1)
    _elapsed_pre_k = time.perf_counter() - start_time
    _kstar_timeout = max(0.01, float(timeout) - _elapsed_pre_k)
    cmd_str = (
        f"{timeout_cmd} {_kstar_timeout} {exec_str} {sas_file} {kstar_search_flag}")
    output = ""
    try:
