...
_TRACKED_SITES = [
    ...
    "slide_site", "mug_site", "milk_site", "tea_site",
...
class KitchenV3Env(BaseEnv):
    """Kitchen environment wrapping dm_control Kitchen."""

    _SEARCHABLE_OBJECTS = ("mug", "milk", "tea")
    _GRASPABLE_OBJECTS = ("mug", "milk", "tea", "keycard")
    _hinge2_keycard_unlocked = False

    # Current env instance for options that need to set MuJoCo state (e.g. hinge2 qpos).
    _current_env: Optional["KitchenV3Env"] = None
    ...
    # Narrow site subtypes reduce task-planning grounding (scheme B): only these
    # objects participate in symbolic pick/transport parameters.
    place_site_type = Type("place_site", ["x", "y", "z"], parent=site_type)
    countertop_site_type = Type("countertop_site", ["x", "y", "z"],
                                 parent=place_site_type)
    keycard_table_site_type = Type("keycard_table_site", ["x", "y", "z"],
                                   parent=place_site_type)
    on_off_type = Type("on_off", ["x", "y", "z", "angle"], parent=site_type)
    hinge_door_type = Type("hinge_door", ["x", "y", "z", "angle", "observed"],
                           parent=on_off_type)
    # Cabinets/microwave where grippables are stocked (excludes hinge1).
    container_pick_site_type = Type(
        "container_pick_site", ["x", "y", "z", "angle", "observed"],
        parent=hinge_door_type)
    # Singleton type for hinge2 (keycard unlock + MoveTool origins).
    hinge2_only_type = Type(
        "hinge2_only", ["x", "y", "z", "angle", "observed"],
        parent=container_pick_site_type)
    ...
    keycard_type = Type("keycard", ["x", "y", "z", "found", "grasped"], parent=grippable_object_type)


    obj_name_to_type = {
        "gripper": gripper_type,
        "hinge2": hinge2_only_type,
        "kettle": kettle_type,
        "microhandle": container_pick_site_type,
        "knob1": knob_type,
        "knob2": knob_type,
        "knob3": knob_type,
        "knob4": knob_type,
        "light": switch_type,
        "slide": container_pick_site_type,
        ...
        "milk": milk_type,
        "tea": tea_type,
        "keycard": keycard_type,
        "countertop": countertop_site_type,
        "keycard_table": keycard_table_site_type,
    }
    ...
    # Class level dictionary to store tea-poured status.
    _tea_poured_status: Dict[str, bool] = {}
    # Pair (tea_name, mug_name) -> True after MoveTeaToMug completes; False otherwise.
    # Avoids geometric false positives (e.g. tea still in microwave vs mug).
    _at_pre_pour_pair_status: Dict[str, bool] = {}
    # Stage flag: tea has been placed on countertop at least once in episode.
    _tea_staged_status: Dict[str, bool] = {}
    ...
    # light_on_thresh = -0.45  # light is On if less than this threshold
    ...
    obj_name_to_pre_push_dpos = {
        ...
        ("light", "on"): (0.1, -0.05, -0.07),
        # ("light", "off"): (-0.1, -0.05, -0.05),
        ("light", "off"): (-0.1, -0.05, -0.07),
    ...
    obj_name_to_pre_pick_dpos = {
        ...
        ("keycard", "keycard_table"): (0.0, 0.0, 0.1),
    }
...
README of that repo suggests!"
        ...
        self._gym_env = mujoco_kitchen_gym.make("FrankaKitchenV3-v1",
    ...
    def goal_predicates(self) -> Set[Predicate]:
        ...
        TeaPoured = self._pred_name_to_pred["TeaPoured"]
    ...
    def create_predicates(cls) -> Dict[str, Predicate]:
        ...
        preds = {
            ...
            Predicate("NotSlide", [cls.hinge_door_type], cls._NotSlide_holds),
            Predicate("IsHinge2", [cls.hinge_door_type], cls._IsHinge2_holds),
            Predicate("NotHinge1", [cls.site_type], cls._NotHinge1_holds),
            Predicate("IsKeycard", [cls.grippable_object_type], cls._IsKeycard_holds),
            Predicate("IsCounterTop", [cls.countertop_site_type],
                      cls._IsCounterTop_holds),
            Predicate("IsKeycardTable", [cls.keycard_table_site_type],
                      cls._IsKeycardTable_holds),
            Predicate("KeycardAt", [cls.grippable_object_type, cls.place_site_type],
                      cls._KeycardAt_holds),
            ...
            Predicate("MugOnCountertop", [cls.mug_type, cls.countertop_site_type],
                      cls._OnTop_holds),
            Predicate("TeaOnCountertop", [cls.tea_type, cls.countertop_site_type],
                      cls._OnTop_holds),
            Predicate("MilkOnCountertop", [cls.milk_type, cls.countertop_site_type],
                      cls._OnTop_holds),
            Predicate("TeaStaged", [cls.tea_type], cls._TeaStaged_holds),
            Predicate("TeaPoured", [cls.tea_type], cls._TeaPoured_holds),
            Predicate(
                "MilkTeaMade",
                [cls.milk_type, cls.tea_type, cls.countertop_site_type],
                cls._OnTop_holds),
            Predicate("Unlocked", [cls.hinge_door_type], cls._Unlocked_holds),
            Predicate("AtPrePour", [cls.gripper_type, cls.grippable_object_type, cls.mug_type], cls._AtPrePour_holds),
        }
    ...
    def types(self) -> Set[Type]:
        return {
            ...
            self.object_type,
            self.site_type,
            self.place_site_type,
            self.countertop_site_type,
            self.keycard_table_site_type,
            self.container_pick_site_type,
            self.hinge2_only_type,
            self.on_off_type,
            self.knob_type,
            self.kettle_type,
            self.switch_type,
            self.hinge_door_type,
            self.surface_type,
            self.mug_type,
            self.milk_type,
            self.tea_type,
    ...
    def _clear_shared_kitchen_belief_state(cls) -> None:
        ...
        cls._tea_poured_status.clear()
        cls._at_pre_pour_pair_status.clear()
        cls._tea_staged_status.clear()
        for object_name in cls._SEARCHABLE_OBJECTS:
            cls._grippable_object_found_status[object_name] = False
        for object_name in cls._GRASPABLE_OBJECTS:
            cls._grippable_object_grasped_status[object_name] = False
        cls._tea_poured_status["tea"] = False
        cls._tea_staged_status["tea"] = False
        cls._grasped_object_relative_pose.clear()
        cls._hinge2_keycard_unlocked = False
    ...
    def reset(self, train_or_test: str, task_idx: int) -> Observation:
        ...
        KitchenV3Env._current_env = self
    ...
    def step(self, action: Action) -> Observation:
        ...
        # Proximity-based unlock must run even with magic options so
        # ``Unlocked(hinge2)`` matches physics after keycard swipe.
        self._maybe_unlock_hinge2_with_keycard()
    ...
    def _maybe_unlock_hinge2_with_keycard(self) -> None:
        ...
        if self._hinge2_keycard_unlocked:
            return
        if not self._grippable_object_grasped_status.get("keycard", False):
            return
        state_info = self.get_object_centric_state_info()
        if "keycard_site" not in state_info:
            return
        keycard_xyz = state_info["keycard_site"][:3]
        hinge2_xyz = self.obj_name_to_xyz["hinge2"]
        if np.linalg.norm(keycard_xyz - hinge2_xyz) < self.keycard_unlock_radius:
            self._hinge2_keycard_unlocked = True
    ...
    def state_info_to_state(cls, state_info: Dict[str, Any]) -> State:
        ...
        # Ensure static site objects are in state_dict (they may not be tracked in state_info)
        for site_name in ("countertop", "keycard_table"):
            site_obj = cls.object_name_to_object(site_name)
            if site_obj in state_dict:
                continue
            site_pos = cls.obj_name_to_xyz.get(site_name, np.array([1.3, 0.5, 1.3]))
            state_dict[site_obj] = {
                "x": float(site_pos[0]),
                "y": float(site_pos[1]),
                "z": float(site_pos[2])
    ...
    @classmethod
    def set_hinge2_unlocked(cls, unlocked: bool = True) -> None:
        """Sync class-level hinge2 unlock with executed ``UnlockHinge`` / keycard motion.

        ``_Unlocked_holds`` for hinge2 reads this flag; without syncing, replanning
        after a successful ``UnlockHinge`` (``MoveToTarget`` keycard to hinge2) can
        still see ``Unlocked(hinge2)`` false and task planning may time out.
        """
        cls._hinge2_keycard_unlocked = bool(unlocked)
    ...
    @classmethod
    def set_tea_poured(cls, tea_name: str = "tea", poured: bool = True) -> None:
        """Set tea-poured symbolic status."""
        cls._tea_poured_status[tea_name] = poured

    @classmethod
    def _at_pre_pour_dict_key(cls, tea_name: str, mug_name: str) -> str:
        return f"{tea_name}:{mug_name}"

    @classmethod
    def set_at_pre_pour(cls, tea_name: str, mug_name: str, value: bool) -> None:
        """Record AtPrePour execution state (paired tea/mug)."""
        cls._at_pre_pour_pair_status[
            cls._at_pre_pour_dict_key(tea_name, mug_name)] = value

    @classmethod
    def clear_at_pre_pour_for_tea(cls, tea_name: str) -> None:
        """Drop any AtPrePour pairs for this tea (e.g. after PlaceTeaOnTable)."""
        prefix = f"{tea_name}:"
        for k in list(cls._at_pre_pour_pair_status.keys()):
            if k.startswith(prefix):
                del cls._at_pre_pour_pair_status[k]
    ...
    def goal_reached(self) -> bool:
        ...
        tea_poured = self._TeaPoured_holds(state, [tea])
        ...
        if goal_desc == ("Make a cup of tea"):
            return tea_poured and tea_on_countertop and mug_on_countertop
    ...
    def _get_tasks(self, num: int,
        ...
        assert CFG.kitchen_goals in [
            "all", "kettle_only", "knob_only", "light_only", "boil_kettle", "put_mug_on_countertop", "make_tea",
    ...
    def set_object_positions_override(self, object_positions):
        ...
        object_positions should be a list of 3 or 4 [x, y, z] lists in the
        order [\"mug\", \"milk\", \"tea\", \"keycard\"]
        (keycard optional). If None is passed,
    ...
    def _setup_new_objects(self, seed: int, train_or_test: str) -> None:
        ...
        object_names = ["mug", "milk", "tea", "keycard"]
        # Backward compatibility: allow legacy 3-object overrides.
        if len(object_positions) < len(object_names):
            object_positions = object_positions + [[-0.7, -0.2, 1.626]]
    ...
    @classmethod
    def _AtPrePour_holds(cls, state: State, objects: Sequence[Object]) -> bool:
        """AtPrePour tracked like TeaPoured — set/cleared by MoveTea options only."""
        del state  # unused
        gripper, tea_obj, mug = objects
        del gripper  # unused
        return cls._at_pre_pour_pair_status.get(
            cls._at_pre_pour_dict_key(tea_obj.name, mug.name), False)
    ...
    def On_holds(cls,
        ...
        if obj.is_instance(cls.knob_type):
            # print(f"Knob {obj.name} angle: {state.get(obj, 'angle')}, threshold: {cls.on_angle_thresh - thresh_pad}")
            return state.get(obj, "angle") < cls.on_angle_thresh - thresh_pad
        if obj.is_instance(cls.switch_type):
            # print(f"Light {obj.name} x: {state.get(obj, 'x')}, threshold: {cls.light_on_thresh - thresh_pad}")
    ...
    @classmethod
    def _Unlocked_holds(cls, state: State, objects: Sequence[Object]) -> bool:
        """Predicate that's necessary for goal specification."""
        obj = objects[0]
        if obj.name == "slide":
            light = cls.object_name_to_object("light")
            return cls.On_holds(state, [light])
        if obj.name == "hinge2":
            return cls._hinge2_keycard_unlocked
        return True

    @classmethod
    def _TeaPoured_holds(cls, state: State, objects: Sequence[Object]) -> bool:
        """Whether tea has been poured according to symbolic execution state."""
        del state  # unused
        tea = objects[0]
        return cls._tea_poured_status.get(tea.name, False)

    @classmethod
    def _TeaStaged_holds(cls, state: State, objects: Sequence[Object]) -> bool:
        """Whether tea has been staged on countertop at least once this episode."""
        tea = objects[0]
        if cls._tea_staged_status.get(tea.name, False):
            return True
        countertop = cls.object_name_to_object("countertop")
        if cls._OnTop_holds(state, [tea, countertop]):
            cls._tea_staged_status[tea.name] = True
            return True
        return False
    ...
    def _IsSlide_holds(cls, state: State, objects: Sequence[Object]) -> bool:
        """Static predicate that identifies the slide container."""
        del state  # unused
        container = objects[0]
        return container.name == "slide"

    @classmethod
    def _NotSlide_holds(cls, state: State, objects: Sequence[Object]) -> bool:
        """Static predicate true for non-slide containers."""
        return not cls._IsSlide_holds(state, objects)

    @classmethod
    def _IsHinge2_holds(cls, state: State, objects: Sequence[Object]) -> bool:
        """Static predicate that identifies hinge2."""
        del state  # unused
        container = objects[0]
        return container.name == "hinge2"

    @classmethod
    def _NotHinge1_holds(cls, state: State, objects: Sequence[Object]) -> bool:
        """Static predicate that identifies non-hinge1 sites."""
        del state  # unused
        site = objects[0]
        return site.name != "hinge1"

    @classmethod
    def _IsKeycard_holds(cls, state: State, objects: Sequence[Object]) -> bool:
        """Static predicate that identifies keycard."""
        del state  # unused
        obj = objects[0]
        return obj.name == "keycard"

    @classmethod
    def _IsKeycardTable_holds(cls, state: State, objects: Sequence[Object]) -> bool:
        """Static predicate that identifies keycard_table site."""
        del state  # unused
        site = objects[0]
        return site.name == "keycard_table"

    @classmethod
    def _IsCounterTop_holds(cls, state: State, objects: Sequence[Object]) -> bool:
        """Static predicate that identifies countertop site."""
        del state  # unused
        site = objects[0]
        return site.name == "countertop"

    @classmethod
    def _KeycardAt_holds(cls, state: State, objects: Sequence[Object]) -> bool:
        """Whether keycard is currently at a target site."""
        keycard, target_site = objects
        if keycard.name != "keycard":
            return False
        # If keycard is grasped, treat it as not being at any site.
        if cls._grippable_object_grasped_status.get("keycard", False):
            return False
        # candidate_sites = ["hinge1", "hinge2", "slide", "microhandle",
        #                    "countertop", "keycard_table"]
        candidate_sites = ["keycard_table"]
        if target_site.name not in candidate_sites:
            return False
        keycard_xyz = np.array([
            state.get(keycard, "x"),
            state.get(keycard, "y"),
            state.get(keycard, "z"),
        ])
        ...
        for site_name in candidate_sites:
            site_obj = cls.object_name_to_object(site_name)
            site_xyz = np.array([
                state.get(site_obj, "x"),
                state.get(site_obj, "y"),
                state.get(site_obj, "z"),
            ])
            distance = float(np.linalg.norm(keycard_xyz - site_xyz))
            if distance < nearest_distance:
                nearest_distance = distance
                nearest_site = site_name
        print(f"nearest_site: {nearest_site}, nearest_distance: {nearest_distance}")
        print(f"keycard_xyz: {keycard_xyz}, target_site_xyz: {site_xyz}")
        return nearest_site == target_site.name and nearest_distance < 0.3
...
def kitchen_maybe_replan_extra_observe_discovery(
    ...
    searchable_names = set(KitchenV3Env._SEARCHABLE_OBJECTS)
