...
def sample_container_for_object(object_name: str, rng: np.random.Generator) -> str:
    ...
    if object_name in ['tea', 'milk']:
        # Tea and Milk: 60% right_hinge_cabinet, 25% slide_cabinet, 15% microwave
        probs = [0.6, 0.25, 0.15]
    elif object_name in ['mug', 'sponge']:
        # Mug and Sponge: 60% slide_cabinet, 25% right_hinge_cabinet, 15% microwave
        probs = [0.25, 0.6, 0.15]
    else:
...
# 四类物体顺序：mug, milk, sponge, tea（与遮挡模式字符串一致）
...
folder_dir = r'D:\Research\Life_Long_Learning\diffusion_proejct\diffusion\diffusion_behavior\mujoco_tea_dataset'
...
with open(mug_position_file, "w") as mug_pos_file, \
    ...
    while any(occlusion_pattern_counts[p] < samples_per_occlusion_pattern for p in ALL_OCCLUSION_PATTERNS):
        ...
        # 为每个物体按概率采样所在容器
        mug_container = sample_container_for_object('mug', rng)
        milk_container = sample_container_for_object('milk', rng)
        sponge_container = sample_container_for_object('sponge', rng)
        tea_container = sample_container_for_object('tea', rng)

        object_to_container = {
            'mug': mug_container,
            'milk': milk_container,
            'sponge': sponge_container,
            'tea': tea_container,
        }
